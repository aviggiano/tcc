...
\begin{frame}{Estado da Arte}{\only<1>{Soluções}\only<2>{Desafios}}
\begin{columns}[c]
\column{.65\textwidth}
...
\only<2>{\structure<2>{\begin{center}\textbf{\textit{Cold start} \\ Esparsidade}\end{center}}}


\textbf{Filtragem baseada em conteúdo (BC)}
\only<2>{\structure<2>{\begin{center}\textbf{Excesso de especialização}\end{center}}}
\par{~}
...
\only<2>{\structure<2>{\begin{center}\textbf{Escalabilidade}\end{center}}}
