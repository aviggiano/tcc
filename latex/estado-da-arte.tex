...
Por exemplo, em um sistema de recomendação de filmes baseado em usuários, a fim de recomendar um título para um usuário $u$, o sistema tenta identificar ``avaliadores'' $v_k^u$ com gostos similares $\ell_{v_k^u i}$ aos do usuário $u$, e então indicam-se os filmes $i$ que os usuários $v_k$ recomendariam. Caso fosse baseado em itens, ele tentaria identificar os itens $j_k^u$ similares  $\ell_{u j_k^u}$ aos itens $i$ adquiridos por $u$. 
