...
Apenas o método UI, para valores de N inferiores ou iguais a 10, atendem ao requisito de \textit{throughput} mínimo de 28 recomendações para cada usuário por segundo. Dado que a base de testes possui $25\%$ do total de usuários, correspondente a 236 clientes para o banco 100k, o tempo de execução deveria ser inferior a $0.15$ min. A fim de melhorar a velocidade das recomendações, a solução mais eficiente é a mudança da linguagem de programação. O uso das linguagens C, C++ ou Python, podem melhorar o desempenho computacional até 500 vezes \cite{benchmarkingR}. 
